...
const publicURL=new URL('explore.html',publication.url).href;
...
await p.locator('#orbit').click();const yaw1=await previewFrame(p).evaluate('window.__JEJU_FLIGHT_PREVIEW__.status.yaw');await p.waitForTimeout(650);const yaw2=await previewFrame(p).evaluate('window.__JEJU_FLIGHT_PREVIEW__.status.yaw');record('Orbit advances actual comparison camera',Math.abs(yaw2-yaw1)>.01);await p.locator('#orbit').click();
