...
for(const name of ['index.html','app.mjs','bridge.mjs','session.mjs','connection.mjs','places.mjs','inspection.mjs','integrity.mjs','CONNECT.md','HANDOFF.md','RELEASE_3_4.md','RELEASE_3_5.md','flight.mjs','explore.html','explore.mjs','explore.css','preview.css'])await fs.copyFile('precision/'+name,path.join(out,name));
const html=await fs.readFile(path.join(out,'index.html'),'utf8');
await fs.writeFile(path.join(out,'index.html'),html.replace(/PRECISION \d+\.\d+/, 'PRECISION '+VERSION.split('-')[0].split('.').slice(0,2).join('.')).replace('<main><aside>','<main><aside><p><a href="explore.html">↗ 전체화면 조감도 탐색 열기</a></p>'));
