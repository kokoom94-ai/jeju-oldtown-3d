...
try{
 ...
 git(['add','--all'],temp);git(['-c','user.name=JEJU Oldtown build','-c','user.email=oldtown-build@users.noreply.github.com','commit','--allow-empty','-m','Stage isolated oldtown '+VERSION+' aerial site; precision unverified'],temp);
...
const publication={version:VERSION,repository:repo,url,homeUrl,explorerUrl:base+'explore.html',siteBranch:SITE_BRANCH,commit,sourceBranch:process.env.GITHUB_REF_NAME,sourceCommit,runId:process.env.GITHUB_RUN_ID,publishedAt:new Date().toISOString(),publicPreviewVerified:check.ok,publicPreviewKeyEntryEnabled:false,htmlCheck:check,plannedDedicatedUrl:PLANNED_URL,pagesSettingChanged:false,providerConfigured:false,sdkLiveTested:false,jejuPrecisionModelsReceived:false,productionReady:false,proxyDeploymentVerified:false,newMainUnchanged:mainBefore===mainAfter,newMainSha:mainAfter,oldRepositoryWriteOperations:0};
