...
if(process.env.GITHUB_REPOSITORY!==repo||process.env.GITHUB_REF_NAME!=='jeju-before-web')throw Error('Wrong repository or branch');
...
const temp=await fs.mkdtemp(path.join(os.tmpdir(),'oldtown-site-'));let added=false,commit;
try{
 ...
 git(['add','--all'],temp);git(['-c','user.name=JEJU Oldtown build','-c','user.email=oldtown-build@users.noreply.github.com','commit','--allow-empty','-m','Stage isolated oldtown 3.3 static site; provider not configured'],temp);
 commit=git(['rev-parse','HEAD'],temp);git(['push','origin','HEAD:refs/heads/'+SITE_BRANCH],temp);
...
const expected=sha(await fs.readFile('_precision_site/index.html'));let check={ok:false};
for(let i=0;i<4;i++){
...
const publication={version:VERSION,repository:repo,url,siteBranch:SITE_BRANCH,commit,sourceBranch:'jeju-before-web',sourceCommit,runId:process.env.GITHUB_RUN_ID,publishedAt:new Date().toISOString(),publicPreviewVerified:check.ok,publicPreviewKeyEntryEnabled:false,htmlCheck:check,plannedDedicatedUrl:PLANNED_URL,pagesSettingChanged:false,providerConfigured:false,sdkLiveTested:false,jejuPrecisionModelsReceived:false,productionReady:false,proxyDeploymentVerified:false,newMainUnchanged:mainBefore===mainAfter,newMainSha:mainAfter,oldRepositoryWriteOperations:0};
