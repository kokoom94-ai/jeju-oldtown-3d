...
if 'fixtureWidgetLayout' not in s:
 marker="key='TEST-ONLY-NOT-A-REAL-KEY-00000';"
 addition="""# fixtureWidgetLayout: fake vw adapter must supply the real engine's widget CSS as the actual SDK does.
...
"""
 assert marker in s;s=s.replace(marker,addition+marker)
 s=s.replace("canvas=frame.locator('canvas').first;box=canvas.bounding_box();check", "canvas=frame.locator('.cesium-widget canvas' if provider else 'canvas').first;box=canvas.bounding_box();measurements.append({'test':tag,'canvasBox':box});print('Canvas '+json.dumps(box),flush=True);check")
p.write_text(s)
